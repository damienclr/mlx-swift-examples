...
public struct FalconH1Configuration: Codable, Sendable {
  ...
  public init(from decoder: Decoder) throws {
      ...
      self.mambaDSSM = try container.decodeIfPresent(Int.self, forKey: .mambaDSSM)
...
private class Mamba2Cache: BaseKVCache {
  ...
  func update(keyStates: MLXArray, valueStates: MLXArray, layerIdx: Int) -> (MLXArray, MLXArray) {
      ...
      if keyCache.count <= layerIdx {
          for _ in keyCache.count ..< layerIdx {
              keyCache.append(MLXArray.zeros([0]))
              valueCache.append(MLXArray.zeros([0]))
          }
  ...
  override var state: [MLXArray] {
      ...
      set {
          ...
          guard newValue.count >= nLayers * 2 else { return }
          for i in 0..<nLayers {
              convStates[i] = newValue[i]
              ssmStates[i] = newValue[i + nLayers]
          }

          let remaining = Array(newValue.dropFirst(nLayers * 2))
          let nKeyCaches = (remaining.count / 2)
          if nKeyCaches > 0 {
              keyCache = Array(remaining[0..<nKeyCaches])
              valueCache = Array(remaining[nKeyCaches..<(nKeyCaches*2)])
          }
      }
  }
  ...
  override var metaState: [String] {
      get {
          [String(offset), String(seqlenOffset), String(hasPreviousState)]
      }
      set {
          if newValue.count >= 3 {
